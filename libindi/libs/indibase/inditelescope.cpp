...
bool Telescope::initProperties()
{
    ...
    IUFillSwitch(&RefreshStausS[0], "REFRESH", "Refresh", ISS_OFF);//zy2018
    IUFillSwitchVector(&RefreshStausSP, RefreshStausS, 1, getDeviceName(), "TELESCOPE_REFRESH_STATUS", "Refresh status", MAIN_CONTROL_TAB,
                       IP_RW, ISR_ATMOST1, 60, IPS_IDLE);
...
bool Telescope::updateProperties()
{
    if (isConnected())
    {
		...
		controller->mapController("REFRESHBUTTON", "Refresh", Controller::CONTROLLER_BUTTON, "BUTTON_4");//zy2018

        //  Now we add our telescope specific stuff
        if (CanGOTO() || CanSync())
            defineSwitch(&CoordSP);
        defineNumber(&EqNP);
		defineSwitch(&RefreshStausSP);//zy2018
        if (CanAbort())
            defineSwitch(&AbortSP);
    ...
    else
    {
		...
		deleteProperty(RefreshStausSP.name);//zy2018
        if (CanAbort())
...
static unsigned long getTimeMs()//zy2018
{
	/*
	...
	/*	struct timeval tv;	
		...
		unsigned long now_ms = (unsigned long)(tv.tv_sec*1000);
		now_ms += tv.tv_usec/1000;
		return now_ms;*/
	
	
	//不受系统时间影响20190911从AIR里复制过来，解决修改手机时间后ra/dec不刷新的问题
	unsigned long ul_ms;
	struct timespec ts;
	clock_gettime(CLOCK_MONOTONIC,&ts);
	ul_ms = ts.tv_sec*1000 + ts.tv_nsec/1000/1000;
	return ul_ms;	
...
{
	static unsigned long time0 = getTimeMs();//zy2018
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        // Refresh status
        ///////////////////////////////////
		if (!strcmp(name, RefreshStausSP.name))//zy2018
        {
            IUResetSwitch(&RefreshStausSP);			
			
			bool rc = true;
			if(getTimeMs() - time0 > 1000)
			{
				time0 = getTimeMs();
		...
		/*   if (!rc)
		   {
            ...
            if (rc)
            {
                RefreshStausSP.s = IPS_OK;
            }
            else
                RefreshStausSP.s = IPS_ALERT;

            IDSetSwitch(&RefreshStausSP, nullptr);
...
void Telescope::SetTelescopeCapability(uint32_t cap, uint8_t slewRateCount)
{
    ...
    if (nSlewRate >= 4)
    {
        ...
        if (nSlewRate == 4)
        {
            strncpy((SlewRateS + (0))->label, "1", MAXINDILABEL);//zy
            strncpy((SlewRateS + (1))->label, "2", MAXINDILABEL);
            strncpy((SlewRateS + (2))->label, "3", MAXINDILABEL);
            strncpy((SlewRateS + (3))->label, "4", MAXINDILABEL);
        }
...
const char *Telescope::LoadParkData()
{
 ...
 //zy 20201210 改成不记住上次的park状态，默认就是unpark。防止出现记住上次的parked，又不支持unpark的情况，导致移动不了
...
void Telescope::processButton(const char *button_n, ISState state)
{
	...
	else if (!strcmp(button_n, "REFRESHBUTTON"))//zy2018
	{
		
			// Invoke parent processing so that Telescope takes care of abort cross-check
			ISState states[1] = { ISS_ON };
			char *names[1]	  = { RefreshStausS[0].name };
			ISNewSwitch(getDeviceName(), RefreshStausSP.name, states, names, 1);
...
void Telescope::processNSWE(double mag, double angle)
{
    ...
    else if (mag > 0.9)
    {
        ...
        // Snap angle to x or y direction if close to corresponding axis (i.e. deviation < 15掳)
