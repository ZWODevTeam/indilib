...
int createLocalConnection(int port)
{
    ...
    if (sock == -1)
    {
        IDLog("Could not create socket\n");
    }
    IDLog("Socket created\n");

 // 	server.sin_addr.s_addr = ((struct in_addr *)(hp->h_addr_list[0]))->s_addr;
 	int ip = getLocalhostIp();
    ...
    {
        IDLog("connect failed. Error\n");
        return -1;
    }

	return sock;
}
int tty_connect_ser2net(int *fd, int port)
{
    if(*fd > 0)
    {
		IDLog("%d, fd > 0, not connect\n", port);
        return TTY_OK;
    }
    ...
    //Connect to remote server
    if((sock = createLocalConnection(port)) < 0)
    {
        IDLog("%d, connect failed. Error\n", port);
        return TTY_PORT_FAILURE;
    }

	*fd = sock;

    IDLog("%d, socket connected\n", port);
    return TTY_OK;

}
int tty_timeout_ms(int fd, int timeout_ms)
{
    ...
    /* wait for 'timeout' seconds */
    tv.tv_sec  = timeout_ms/1000;//2400 = 2sec + 400*1000
    tv.tv_usec = timeout_ms%1000*1000;

    /* Wait till we have a change in the Port FD status */
    retval = select(fd + 1, &readout, NULL, NULL, &tv);

    /* Return 0 on successful Port FD change */
    if (retval > 0)
        return TTY_OK;
    /* Return -1 due to an error */
    else if (retval == -1)
...
int tty_read_ms(int fd,  char *buf, int buflen, int timeout_ms, int *nbytes_read)
{
...
//!!!参照ser2net的超时策略: 第一个数据等待长时间，后续的数据等待短时间
    while (buflen > *nbytes_read)
    {
        if ((err = tty_timeout_ms(fd, timeout_ms)))//第一个数据等待长时间1000ms
        {
            return err;
        }
		timeout_ms = 3;//后面的数据等待短时间3ms

        bytesRead = read(fd, buffer + (*nbytes_read), ((uint32_t)numBytesToRead));
		if((bytesRead == 0 || (errno != EINTR && bytesRead < 0)))//netPort > -1 && 
		{
...
int tty_write(int fd, const char *buf, int nbytes, int *nbytes_written)
{
    ...
    *nbytes_written = 0;
	...
	if(!udp)//通过ser2net连接的 //每次都重新连接的话延时较大，导星容易抖动，因此改用长连接，但可能长时间无操作被服务端关闭，需要重新连
	{
		char readBuffer[257]={0};
		int nbytes_read = 0;
		int err = tty_read_ms(fd, readBuffer, 257, 1, &nbytes_read);
		if (err != TTY_OK && err != TTY_TIME_OUT)//检查连接是否正常，可能被服务端关闭了，则重新连接
	    {	      
			IDLog("err = %d, connection is closed by svr, reconnect\n", err);
			if(tty_disconnect(fd) == TTY_OK)
				fd = -1;
			if(tty_connect_ser2net(&fd, 4030) != TTY_OK)
			{
				IDLog("reconnect failed\n");
				return TTY_PORT_FAILURE;		
			}
	    }
	}
...
int tty_read(int fd, char *buf, int nbytes, int timeout, int *nbytes_read)
{
    ...
    *nbytes_read  = 0;
    ...
    while (numBytesToRead > 0)
    {
		...
		if(!udp)
		{
			if((bytesRead == 0 || (errno != EINTR && bytesRead < 0)))//防止设备被拔掉后死循环
		            return TTY_READ_ERROR;
		}
        else if (bytesRead < 0)
            return TTY_READ_ERROR;
...
int tty_nread_section(int fd, char *buf, int nsize, char stop_char, int timeout, int *nbytes_read)
{
    ...
    // For Gemini
	//zy 20210112从1.8.8对比加|| ttySkyWatcherUdpFormat
    if (ttyGeminiUdpFormat || ttySkyWatcherUdpFormat)
        return tty_read_section(fd, buf, stop_char, timeout, nbytes_read);
...
void tty_read_out_socket(int fd)
{
	char readBuffer[256]={0};
	int nbytes_read;
	do{
		nbytes_read = 0;
		tty_read(fd, readBuffer, 256, 0, &nbytes_read);		
	}while(nbytes_read > 0);
}
...
int tty_connect(const char *device, int bit_rate, int word_size, int parity, int stop_bits, int *fd)
{
	...
	char szCmd[256];
	sprintf(szCmd, "{\"method\":\"set_settings\",\"params\":[{\"baudrate\":%d,\"word_size\":%d,\"parity\":%d,\"stop_bits\":%d}],\"id\":1108}\n"
		, bit_rate, word_size, parity, stop_bits);
	SendSocketCmd(4040, szCmd, 500, "\"id\":1108", szRes, 255, &iResLen);

	return tty_connect_ser2net(fd, 4030);
/*#ifdef _WIN32
    return TTY_PORT_FAILURE;
...
{	
    ...
    int err = close(fd);//:Bad file descriptor
	...
	char szRes[256];
	int iResLen= 0;
	SendSocketCmd(4040, "{\"method\":\"disconnect\",\"id\":1338}\n", 500, "\"id\":1338", szRes, 255, &iResLen);
...
向文件描述符写入，全部写入或失败才结束
int handle, 描述符
const char *buffer, 写入缓冲
long length，写入长度
返回：负数为失败，0为成功
***/
int my_write(int handle, const char *buffer, long length) {
	long remains = length;
	while (1) {
...
返回：NET_OK为成功
***/
static int selectTimeoutMs(int handle, int timeout_ms)
{
    ...
    /* Wait till we have a change in the Port FD status */
    retval = select(handle + 1, &readout, NULL, NULL, &tv);
...
int buflen, 缓冲区长度
int timeout_ms, 超时毫秒数
int *nbytes_read, 读出长度
const char* pszSubstring，匹配的子串
返回：NET_OK为成功
***/
static int readNet(int socket, char *buf, int buflen, int timeout_ms, int *nbytes_read, const char* pszSubstring)
{
	
    if (socket == INT_DEFAULT)
    {
        return NET_ERRNO;
    }
...
//!!!参照ser2net的超时策略: 第一个数据等待长时间，后续的数据等待短时间
    ...
    {
//		fprintf(stderr,"%s\n", buffer);
        if ((err = selectTimeoutMs(socket, timeout_ms)))//第一个数据等待长时间1000ms
        {
            return err;
        }		

        bytesRead = read(socket, buffer + (*nbytes_read), ((uint32_t)numBytesToRead));

		if(pszSubstring)
		{
			if(strstr(buffer, pszSubstring))//找到指定的substring后等待时间才变短
			{
				timeout_ms = 3;//后面的数据等待短时间3ms
			...
			else
			{
				...
				if(p0)
				{
					//012345678\ndfgdfg
					//dfgdfg___________
					int cpLen = buf + strlen(buf) - p0 - 1;
					int deleteLen = p0 - buf + 1;//9 - 0 + 1 ->10
					if(cpLen != 0)
					{
						memcpy(buf, p0 + 1, cpLen);					
					}
					memset(buf + cpLen, 0, buflen - cpLen);
					*nbytes_read -= deleteLen;
				}	
		...
		if((bytesRead == 0 || (bytesRead < 0 && errno != EINTR)))//netPort > -1 && 
		{
        ...
        *nbytes_read += bytesRead;
        numBytesToRead = buflen - (*nbytes_read);	
...
char *buf, 	缓冲区	
int buflen, 	缓冲区长度
int *nbytes_read, 回复的长度
const char* pszSubstring,	特征子串

*/
int sendNetCmd(int socket, const char* pszCmd, int timeout_ms, char *buf, int buflen, int *nbytes_read, const char* pszSubstring)
{
	if(pszCmd[strlen(pszCmd) - 1] != '\n')//0 1 2 3 4
	{
		fprintf(stderr,"must end with \\n\n");
		return NET_ERRNO;
	}
	if(my_write(socket, pszCmd, strlen(pszCmd)) < 0)
		return NET_ERRNO;

	fprintf(stderr,"readNet: %s>\n", pszCmd);
	
	int errcode = readNet(socket, buf, buflen, timeout_ms, nbytes_read, pszSubstring);

//含有欢迎行，要去除
	if(pszSubstring)
	{
		//将目标子串前'\n'之前的内容清除
		char* p0 = strstr(buf, pszSubstring);
		if(p0)
		{
			//asfjlskflsjdf\n012345678\n
			while(p0 >= buf)
	...
	return (*nbytes_read > 0 && (errcode == NET_OK || errcode == NET_TIME_OUT)) ? NET_OK : NET_ERRNO;
		
}
/***
字符串分割
const char* strHead, 起始，不包含
const char* strTail, 结束，不包含
char* strInput,源字串，返回后内容会修改

***/

int strSplit(const char* strHead, const char* strTail, char* strInput)
{
	char* pszHead = 0;
	if(strHead)
	{
	...
	if(pszHead)//ASI
	{				
		if(pszHead - strInput >= (int)strlen(strInput))//ZWO ASI 7 - 0 = 7//超出范围了
			return -1;
		else
		{
			char* pszTail = 0;
...
int SendSocketCmd(int port, const char* pszCmd, int timeout_ms, const char* pszSubstring, char *buf, int buflen, int *nbytes_read)
{
		
	int socket = INT_DEFAULT;
	
	socket = createLocalConnection(port);
	if(socket < 0)
	{
		fprintf(stderr, "reconnect socket failed\n");
		return NET_ERRNO;
	}	

	int Ret = NET_ERRNO;

	if(sendNetCmd(socket,  pszCmd, timeout_ms, buf, buflen, nbytes_read,  pszSubstring) == NET_OK)
	{		
		if(strSplit("\"result\":", ",\"id\"", buf) == 0)//{"jsonrpc":"2.0","result":[12.734,90.000],"id":13}
			Ret = NET_OK;
	}
