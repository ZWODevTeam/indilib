...
LX200_OnStep::LX200_OnStep() : LX200Generic(), FI(this)
{
    ...
    setLX200Capability(LX200_HAS_TRACKING_FREQ | LX200_HAS_SITES | LX200_HAS_ALIGNMENT_TYPE | LX200_HAS_PULSE_GUIDING | LX200_HAS_PRECISE_TRACKING_FREQ);
 	...
 	int arr_len = sizeof(slewRates)/sizeof(S_SLEWRATE);
    SetTelescopeCapability(GetTelescopeCapability() | TELESCOPE_CAN_CONTROL_TRACK | TELESCOPE_HAS_PEC | TELESCOPE_HAS_PIER_SIDE | TELESCOPE_HAS_TRACK_RATE, arr_len );
...
//     IUFillText(&OSNAlignErrT[7], "7", "# of Align Stars", "Not Updated");
	...
	for(int i = 0; i < arr_len; i++)
		strncpy(SlewRateS[i].label, slewRates[i].label, MAXINDILABEL);
	
    IUResetSwitch(&SlewRateSP);
    // the default
    SlewRateS[arr_len - 1].s = ISS_ON;
...
bool LX200_OnStep::SetSlewRate(int index)
{
	if(isSimulation())
		return true;	
    ...
    if (tty_write_string(PortFD, slewRates[index].rate_cmd, &nbytes_write) != TTY_OK)
        return false;      
