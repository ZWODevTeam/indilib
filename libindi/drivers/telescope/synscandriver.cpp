...
bool SynscanDriver::Park()
{
    ...
    {
        TrackState = SCOPE_PARKING;//zy 20201210 设置了这个状态，刷新状态refresh才会变成park状态
...
//		IsWest = true;
 ...
 /*   if (longitude > 180)
    ...
    LOGF_INFO("Update location to latitude %d:%d:%1.2f(neg:%d) longitude %d:%d:%1.2f(neg:%d)",
              p2.lat.degrees, p2.lat.minutes, p2.lat.seconds, p2.lat.neg, p2.lng.degrees, p2.lng.minutes, p2.lng.seconds, p2.lng.neg);
    ...
    cmd[8] = (p2.lng.neg == 0) ? 0 : 1;//IsWest ? 1 : 0;
    
 //   LOGF_INFO("cmd[8]: %d", cmd[8]);
