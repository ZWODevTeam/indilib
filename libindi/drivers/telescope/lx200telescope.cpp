...
bool LX200Telescope::updateLocation(double latitude, double longitude, double elevation)
{
	...
	/*zy20201215 lx200是东负西正,但我们app发送的是东正西负，需要反一下
	修复Losmandy Gemini设置地理位置失败的问题
	Losmandy Gemini-2 and Gemini-1 Telescope Mount Controllers
	:Sg{+-}<ddd>*<mm># 	1 if valid 	Sets the longitude of the observing site to ddd degrees and mm minutes. 
	The longitude has to be specified positively for western latitudes (west of Greenwich, the plus sign may be omitted) 
	and negatively for eastern longitudes. Alternatively, 360 degrees may be added to eastern longitudes.
	*/
    if (!isSimulation() && setSiteLongitude(PortFD, longitude > 0 ? 360.0 - longitude : -longitude) < 0)
    {
