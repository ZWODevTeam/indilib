...
bool CelestronGPS::updateProperties()
{
    if (isConnected())
    {
        ...
        if (driver.get_firmware(&fwInfo))
        {
			...
			if(fwInfo.celestronTrackMode == CTM_EQN)
				fprintf(stderr, "updateProperties0->N\n");
			else if(fwInfo.celestronTrackMode == CTM_EQS)
				fprintf(stderr, "updateProperties0->S\n");
			else
				fprintf(stderr, "updateProperties0->N/A\n");
        }
        else
        {
            fwInfo.Version = "Invalid";
        ...
        if (checkMinVersion(2.3, "track mode"))
        {
            ...
            if (driver.get_track_mode(&ctm))
            {
                if (ctm != CTM_OFF)
                {
					...
					if(fwInfo.celestronTrackMode == CTM_EQN)
						fprintf(stderr, "updateProperties1->N\n");
					else if(fwInfo.celestronTrackMode == CTM_EQS)
						fprintf(stderr, "updateProperties1->S\n");
					else
						fprintf(stderr, "updateProperties1->N/A\n");
                ...
                else
                {
					...
					fprintf(stderr, "init->%s\n", TrackState == SCOPE_IDLE ? "idle" : "park");
                }
            }
            else
            {
     ...
     //       TrackState = SCOPE_TRACKING;
            ...
            if (driver.get_track_mode(&ctm))
            {
               ...
               /*     //IUResetSwitch(&TrackSP);
                    ...
                    CelestronTrackModeSP.s      = IPS_OK;*/                
                    TrackState = SCOPE_TRACKING;
			   		fprintf(stderr, "slew->track\n");
                }
				else
				{
		     		TrackState = SCOPE_IDLE;
			 		fprintf(stderr, "slew->idle\n");
				}
            }
			else
				fprintf(stderr, "get_track_mode fail\n");
